...
usersRouter
  ...
  .post(jsonParser, (req, res, next) => {
    ...
    bcrypt.hash(newUser.password, 10).then((hash) => {
      ...
      UsersService.insertUser(req.app.get("db"), hashedUser)
        .then((user) => {
...
usersRouter
  ...
  .patch(jsonParser, (req, res, next) => {
    ...
    if (req.body.password !== undefined) {
      bcrypt.hash(req.body.password, 6).then((hashPass) => {
        const hashedUser = {
          password: hashPass,
        };
...
usersRouter
  ...
  .all((req, res, next) => {
    UsersService.getByUsername(req.app.get("db"), req.params.username)
      .then((user) => {
        if (user) {
          return res.status(404).json({
...
usersRouter
  ...
  .all((req, res, next) => {
    UsersService.getIdOnly(req.app.get("db"), req.params.username)
      .then((user) => {
        if (!user) {
          return res.status(404).json({
            error: { message: `User doesn't exist` },
...
usersRouter
  .route("/adminPage/all")
  .all((req, res, next) => {
    UsersService.adminGetAllUsers(req.app.get("db"))
      .then((user) => {
        if (user.length === 0) {
          return res.status(404).json({
